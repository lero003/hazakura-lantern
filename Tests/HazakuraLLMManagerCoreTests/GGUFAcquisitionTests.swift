...
final class GGUFAcquisitionTests: XCTestCase {
    ...
    func testClientSearchNormalizesBlankAdvisoryDisplayMetadata() async throws {
        let session = makeSession { _ in
            (
                200,
                Data("""
        ...
        XCTAssertEqual(results.map(\.id), ["owner/blank-advisory-GGUF", "owner/padded-advisory-GGUF"])
        XCTAssertNil(results[0].author)
        XCTAssertNil(results[0].lastModified)
        XCTAssertEqual(results[0].tags, ["gguf"])
        XCTAssertEqual(results[1].author, "owner")
        XCTAssertEqual(results[1].lastModified, "2026-05-28T00:00:00.000Z")
    }
    ...
    func testClientSearchNormalizesReadableTags() async throws {
        let session = makeSession { _ in
            (
                200,
                Data("""
        ...
        XCTAssertEqual(results.map(\.id), ["owner/padded-tags-GGUF"])
        XCTAssertEqual(results[0].tags, ["gguf", "qwen"])
    }
    ...
    func testDownloaderRejectsResumeWhenContentRangeTotalDisagreesWithExpectedSize() async throws {
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent("hazakura-gguf-download-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: workspace) }
        try FileManager.default.createDirectory(at: workspace, withIntermediateDirectories: true)
        ...
        do {
            ...
            XCTFail("Expected resume responses with mismatched server totals to fail before appending bytes.")
        } catch let error as GGUFAcquisitionError {
            XCTAssertEqual(error, .incompleteDownload(expectedBytes: 11, actualBytes: 12))
        }
