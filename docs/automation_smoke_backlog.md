...
- `swift test` passed: 306 XCTest tests, 0 failures.
- `swift build --disable-sandbox` passed.
- `./script/build_and_run.sh --verify` could not provide launch evidence in
  this host session because Launch Services returned `kLSNoExecutableErr`.
  A control `open -W -n /System/Applications/Calculator.app` probe failed with
  the same error, so treat this run as a host Launch Services blocker rather
  than Lantern-specific app-bundle evidence. `./script/build_and_run.sh --stop`
  was run after probing.
- No code change was made in this pass; the existing working tree was already
  dirty before the automation started.

Previous source-verification result (2026-05-28 no-download GGUF public API
smoke pass):
...
- GGUF Acquisition downloads now treat non-positive expected byte counts and
  `Content-Length` values as unknown metadata, keeping malformed size values
  out of progress and completion checks.
...
- GGUF Acquisition repository tree parsing now trims and case-normalizes
  compatible file-entry `type` values such as `" FILE "` while continuing to
...
- GGUF Acquisition search parsing now treats negative advisory `downloads` /
  `likes` counts as unknown metadata, keeping compatible public repository
  results selectable without trusting impossible count values.
- GGUF Acquisition public API parsing now treats string-valued or malformed
  ...
  `size` as optional, keeping otherwise compatible repositories and `.gguf`
  files selectable without starting a live download.
...
- GGUF Acquisition downloader coverage now proves an unexpected `206`
  `Content-Range` response that starts after byte `0` fails before creating a
  misleading `.part` suffix when no resume file exists.
- GGUF Acquisition resumed downloader coverage now proves a short `206`
  response is rejected when its valid `Content-Range` header exposes the final
  total byte count, even when the repository tree did not provide file size
  metadata.
...
- Smoke Console response parsing now accepts compatible `message.reasoning`
  ...
  behavior.
- Smoke Console metric parsing now treats compatible `usage.input_tokens` and
  `usage.output_tokens` fields as runtime-reported usage, while keeping
  standard `prompt_tokens` / `completion_tokens` values authoritative when both
  shapes are present.
- Smoke Console metric parsing now preserves explicitly reported zero token
  counts from standard `usage` and compatible `llama-server` `timings` payloads
  instead of replacing them with approximate fallback metrics.
