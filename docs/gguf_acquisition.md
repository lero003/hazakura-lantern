...
compatible safe repository id. Blank or padded `author` and date strings are
trimmed or ignored before they become UI chips, and mixed `tags` arrays keep
readable string tags after trimming whitespace and dropping blank entries while
ignoring malformed tag entries. Malformed search/tree array items, search
