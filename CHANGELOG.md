...
- Added a repo-local warning-expected DMG preview lane with a plain
  ...
  identity verification, download-friendly checksum file, checklist, and draft
  release notes.
...
- Hardened GGUF Acquisition search parsing so string-valued Hugging Face
  `gated` metadata such as `auto`, `manual`, or `false` does not break public
  repository search results.
- Hardened GGUF Acquisition public API parsing so string-valued or malformed
...
- Hardened GGUF Acquisition downloads so non-positive expected byte counts and
  `Content-Length` values are treated as unknown metadata instead of poisoning
  progress or completion checks.
...
- Hardened GGUF Acquisition downloads so invalid `Content-Range` totals are
  rejected before appending resume bytes, keeping the existing `.part` file
  available for an explicit retry.
...
- Added Smoke Console response parsing for compatible `output_text` content
  parts, keeping text-part smoke evidence readable when a local runtime uses
  response-style labels inside `message.content`.
...
- Improved Smoke Console HTTP error snippets so structured `error`, `detail`,
  ...
  FastAPI-style detail arrays, code-only error objects, and objects with blank
  `message` fields plus readable fallback codes, are shown as readable bounded
